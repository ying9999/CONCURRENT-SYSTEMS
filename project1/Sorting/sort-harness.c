...
void * david_alloc(size_t nbytes) {
  const int write_messages = 0;

  if ( write_messages == 1 ) {
    fprintf(stderr, "Attempting to allocate %lu bytes\n", nbytes);
  }
  if ( nbytes <= 0 ) {
    fprintf(stderr, "Error: Trying to allocate zero or negatives bytes: %lu\n", nbytes);
    exit(1);
  }
  const size_t max_bytes = 1 << 30;
  if ( nbytes > max_bytes ) {
    fprintf(stderr, "Error: Trying to allocate more than %lu bytes: %lu\n", max_bytes, nbytes);
    exit(1);
  }
  void * result = malloc(nbytes);
  if ( result == NULL ) {
    fprintf(stderr, "Error: Allocation of %lu bytes failed; no memory allocated\n", nbytes);
    exit(1);
  }
  return result;
}
...
float * read_in(char filename[], int * ptr2size)
{
  ...
  sscanf(line, "%d", &size);
  a = david_alloc(sizeof(long long) * size);
...
{
  float * result = david_alloc(sizeof(float) * size);
  int i;
